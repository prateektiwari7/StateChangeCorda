...
private class UpgradeContractStateClient {
    ...
    fun main(args: Array<String>) {
     ...
     //   require(args.size == 2) { "Usage: TemplateClient <PartyA RPC address> <PartyB RPC address>" }
        ...
        val partyAProxy = client.start(rpcUsername, rpcPassword).proxy


        val nodeAddress1 = parse("localhost:10009")
        val rpcUsername1 = "user1"
        val rpcPassword1 = "test"
        val client1 = CordaRPCClient(nodeAddress1)
        val partyBProxy = client1.start(rpcUsername1, rpcPassword1).proxy
